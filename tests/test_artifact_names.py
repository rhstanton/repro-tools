...
def test_an_empty_checksums_file_fails(tmp_path):
    """A file whose presence is the whole signal must be checked for content.

    With no entries the verification loop never runs and the check reports
    success having pinned nothing. A zero-byte data/CHECKSUMS.txt is easy to
    create by accident -- a shell redirection makes one before the command
    writing it has produced a single byte.
    """
    from repro_tools.presubmit import PreSubmitChecker
    ...
    c = PreSubmitChecker(tmp_path)
    c.check_data_files()
    result = next(r for r in c.results if r.name == "Data Checksums")
    assert not result.passed, "an empty checksums file was accepted"
    assert "no files" in result.message


def test_a_comments_only_checksums_file_fails(tmp_path):
    """Same defect wearing a hat: every line filtered out leaves nothing."""
    from repro_tools.presubmit import PreSubmitChecker

    data = tmp_path / "data"
    data.mkdir()
    (data / "CHECKSUMS.txt").write_text("# generated by hand\n#\n\n")
    c = PreSubmitChecker(tmp_path)
    c.check_data_files()
    result = next(r for r in c.results if r.name == "Data Checksums")
